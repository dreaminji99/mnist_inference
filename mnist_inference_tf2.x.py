'''This is inference code for mnist dataset '''

from __future__ import print_function
import tensorflow.keras
from tensorflow.keras.datasets import mnist
from tensorflow.keras.models import Sequential
from tensorflow.keras.layers import Dense, Dropout, Flatten
from tensorflow.keras.layers import Conv2D, MaxPooling2D
from tensorflow.keras import backend as K
import matplotlib.pyplot as plt
from PIL import Image
import numpy as np
import os
import tensorflow as tf

import pandas as pandas

# 기존 saved_model/ 폴더 불러오기
#model = tf.keras.models.load_model("saved_model/")  # Keras 2.x에서는 가능

# 새로운 .keras 포맷으로 저장
#model.save("saved_model_converted.keras")  # Keras 3 호환 파일



# Recreate the exact same model, including its weights and the optimizer

model = tf.keras.models.load_model('saved_model/')
model.save('saved_model.keras')
#from keras.layers import TFSMLayer

#model = TFSMLayer('saved_model/', call_endpoint='serving_default')

# Show the model architecture
model.summary()

##-- Model Test using Test datasets
print()
print("----Actual test for digits----")

#mnist_label_file_path =  "dataset_test/testimgs"
#mnist_label = open(mnist_label_file_path, "r")

cnt_correct = 0
for index in range(10):
	#-- read a label
	label = index	
	img_path = f"dataset_test/testimgs/{index+1}.png"
    #label = mnist_label readline()
	#print(label)
	#-- formatting the input image (image data)
	img = Image.open(img_path).convert("L")
	img = img.resize((28,28))
	im2arr = np.array(img)
	im2arr = im2arr.reshape(1,28,28,1)

	# Predicting the Test set results
	y_pred = model.predict(im2arr)	#<-- 7 or 4
	pred_label = np.argmax(y_pred) # 수정 1개 했음음
 
	print()
	print(f"label={label} --> predicted label = {pred_label}")
   
	#print("label = {} --> predicted label= {}".format(label, pred_label))

	#-- compute the accuracy of the preditcion
	if int(label)==pred_label:
		cnt_correct += 1
        
#-- Final accuracy
Final_acc = cnt_correct/10
print()
print("Final test accuray: %f" %Final_acc)
print()
print('****tensorflow version****:',tf.__version__)
print()

data = {
    '이름': ['구민지'],
    '학번': [2111112],
    '학과': ['인공지능공학부']
}

df = pandas.DataFrame(data)
print(df)